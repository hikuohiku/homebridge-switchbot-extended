...
import { AirConditioner } from './irdevice/myairconditioner.js';
...
export class SwitchBotPlatform implements DynamicPlatformPlugin {
  ...
  constructor(log: Logging, config: SwitchBotPlatformConfig, api: API) {
    ...
    try {
      ...
      this.errorLog(
        `Verify Config, Error Message: ${e.message}, Submit Bugs Here: ` +
    ...
    this.api.on('didFinishLaunching', async () => {
      ...
      try {
        if (
          this.config.credentials?.openToken &&
          ...
          this.errorLog(
            '"secret" config is not populated, you must populate then please restart Homebridge.',
          );
        } else {
        ...
        this.errorLog(
          `Failed to Discover, Error Message: ${e.message}, Submit Bugs Here: ` +
            'https://tinyurl.com/SwitchBotBug',
        );
  ...
  async setupMqtt(): Promise<void> {
    if (this.config.options?.mqttURL) {
      try {
        ...
        this.mqttClient = await connectAsync(
          this.config.options?.mqttURL,
          this.config.options.mqttOptions || {},
        );
        await this.debugLog(
        ...
        if (!this.config.options?.webhookURL) {
          ...
          await this.infoLog(
            `Webhook is configured to be received through ${this.config.options.mqttURL}/homebridge-switchbot/webhook.`,
          );
          this.mqttClient.subscribe('homebridge-switchbot/webhook/+');
          this.mqttClient.on('message', async (topic: string, message) => {
            try {
              await await this.debugLog(
                `Received Webhook via MQTT: ${topic}=${message}`,
              );
  ...
  async setupwebhook() {
    ...
    if (this.config.options?.webhookURL) {
      ...
      try {
        ...
        this.webhookEventListener = createServer(
                  ...
                  try {
                    ...
                    if (this.config.options?.mqttURL) {
                      ...
                      this.mqttClient?.publish(
                        `homebridge-switchbot/webhook/${mac}`,
                        `${JSON.stringify(body.context)}`,
                        options,
                      );
                    }
                    this.webhookEventHandler[body.context.deviceMac]?.(
                      body.context,
                    );
                  } catch (e: any) {
      ...
      try {
        ...
        if (statusCode !== 200 || response?.statusCode !== 100) {
          this.errorLog(
            `Failed to configure webhook. Existing webhook well be overridden. HTTP:${statusCode} API:${response?.statusCode} ` +
      ...
      try {
        ...
        if (statusCode !== 200 || response?.statusCode !== 100) {
          this.errorLog(
            `Failed to update webhook. HTTP:${statusCode} API:${response?.statusCode} message:${response?.message}`,
          );
        }
      } catch (e: any) {
      ...
      try {
        ...
        if (statusCode !== 200 || response?.statusCode !== 100) {
          this.errorLog(
            `Failed to query webhook. HTTP:${statusCode} API:${response?.statusCode} message:${response?.message}`,
          );
        } else {
      ...
      this.api.on('shutdown', async () => {
        try {
          ...
          const response: any = await body.json();
          await this.debugLog(
            `deleteWebhook: url:${url}, body:${JSON.stringify(response)}, statusCode:${statusCode}`,
          );
          if (statusCode !== 200 || response?.statusCode !== 100) {
            this.errorLog(
              `Failed to delete webhook. HTTP:${statusCode} API:${response?.statusCode} message:${response?.message}`,
            );
          } else {
  ...
  async setupBlE() {
    if (this.config.options?.BLE) {
      ...
      if (switchbot === undefined) {
        await this.errorLog(
          `wasn't able to establish BLE Connection, node-switchbot: ${switchbot}`,
        );
      } else {
        ...
        this.api.on('shutdown', async () => {
          try {
            ...
            this.errorLog(
              `Failed to stop Platform BLE scanning. Error:${e.message}`,
            );
          }
        });
      }
  ...
  async verifyConfig() {
    ...
    if (!this.config.credentials && !this.config.options) {
      ...
      if (!this.config.credentials?.token) {
        ...
        this.debugWarnLog(
          'Cloud Enabled SwitchBot Devices & IR Devices will not work',
        );
      }
      if (this.config.credentials?.token) {
    ...
    if (this.config.credentials?.token) {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          await this.debugWarnLog(
            `devicesAPI Body: ${JSON.stringify(devicesAPI.body)}`,
          );
          await this.debugWarnLog(
            `devicesAPI StatusCode: ${devicesAPI.statusCode}`,
          );
          if (
            (statusCode === 200 || statusCode === 100) &&
            (devicesAPI.statusCode === 200 || devicesAPI.statusCode === 100)
          ) {
            await this.debugErrorLog(
              `statusCode: ${statusCode} & devicesAPI StatusCode: ${devicesAPI.statusCode}`,
            );
            // SwitchBot Devices
            const deviceLists = devicesAPI.body.deviceList;
            await this.debugWarnLog(
              `DeviceLists: ${JSON.stringify(deviceLists)}`,
            );
            await this.debugWarnLog(
              `DeviceLists Length: ${deviceLists.length}`,
            );
            if (!this.config.options?.devices) {
              await this.debugLog(
                `SwitchBot Device Config Not Set: ${JSON.stringify(this.config.options?.devices)}`,
              );
              if (deviceLists.length === 0) {
                await this.debugLog(
                  `SwitchBot API Has No Devices With Cloud Services Enabled: ${JSON.stringify(devicesAPI.body)}`,
                );
              } else {
              ...
              await this.debugLog(
                `SwitchBot Device Config Set: ${JSON.stringify(this.config.options?.devices)}`,
              );
              if (deviceLists.length === 0) {
                await this.debugLog(
                  `SwitchBot API Has No Devices With Cloud Services Enabled: ${JSON.stringify(devicesAPI.body)}`,
                ...
                await this.debugLog(
                  `SwitchBot Devices: ${JSON.stringify(devices)}`,
                );
                for (const device of devices) {
                  if (!device.deviceType) {
                    device.deviceType = device.configDeviceType;
                    this.errorLog(
                      `API has displaying no deviceType: ${device.deviceType}, So using configDeviceType: ${device.configDeviceType}`,
                    );
                  }
                  if (device.deviceType) {
                    if (device.configDeviceName) {
            ...
            if (devicesAPI.body.deviceList.length !== 0) {
              this.infoLog(
                `Total SwitchBot Devices Found: ${devicesAPI.body.deviceList.length}`,
              );
            } else {
              await this.debugLog(
                `Total SwitchBot Devices Found: ${devicesAPI.body.deviceList.length}`,
            ...
            if (!this.config.options?.irdevices) {
              await this.debugLog(
                `IR Device Config Not Set: ${JSON.stringify(this.config.options?.irdevices)}`,
              );
              ...
              await this.debugLog(
                `IR Device Config Set: ${JSON.stringify(this.config.options?.irdevices)}`,
              );
              const irDeviceConfig = this.config.options?.irdevices;

              const mergeIRBydeviceId = (
                a1: { deviceId: string }[],
                a2: any[],
              ) =>
                a1.map((itm: { deviceId: string }) => ({
                  ...a2.find(
                    (item: { deviceId: string }) =>
                      item.deviceId.toUpperCase().replace(/[^A-Z0-9]+/g, '') ===
                        itm.deviceId.toUpperCase().replace(/[^A-Z0-9]+/g, '') &&
                      item,
                  ),
                  ...itm,
                }));
            ...
            if (devicesAPI.body.infraredRemoteList.length !== 0) {
              this.infoLog(
                `Total IR Devices Found: ${devicesAPI.body.infraredRemoteList.length}`,
              );
            } else {
              await this.debugLog(
                `Total IR Devices Found: ${devicesAPI.body.infraredRemoteList.length}`,
            ...
            if (statusCode === 500) {
              retryCount++;
              this.infoLog(
                `statusCode: ${statusCode} Attempt ${retryCount} of ${maxRetries}`,
              );
          ...
          retryCount++;
          this.debugErrorLog(
            `Failed to Discover Devices, Error Message: ${JSON.stringify(e.message)}, Submit Bugs Here: ` +
              'https://tinyurl.com/SwitchBotBug',
          );
      ...
      await this.debugLog(
        `SwitchBot Device Manual Config Set: ${JSON.stringify(this.config.options?.devices)}`,
      );
      const deviceConfigs = this.config.options?.devices;
  ...
  private async createDevice(device: device & devicesConfig) {
    switch (device.deviceType!) {
      case 'Humidifier':
        await this.debugLog(
          `Discovered ${device.deviceType}: ${device.deviceId}`,
        );
      ...
      case 'Curtain3':
        await this.debugLog(
          `Discovered ${device.deviceType} ${device.deviceName}: ${device.deviceId}`,
        );
      ...
      case 'remote with screen+':
        await this.debugLog(
          `Discovered ${device.deviceType}: ${device.deviceId}, is currently not supported, device: ${JSON.stringify(device)}`,
        );
        break;
      default:
        this.warnLog(
          `Device: ${device.deviceName} with Device Type: ${device.deviceType}, is currently not supported.`,
  ...
  private async createIRDevice(device: irdevice & devicesConfig) {
    ...
    switch (device.remoteType) {
      ...
      case 'DIY Speaker':
        await this.debugLog(
          `Discovered ${device.remoteType}: ${device.deviceId}`,
        );
      ...
      default:
        ...
        this.warnLog(
          `Device: ${device.deviceName} with Device Type: ${device.remoteType}, is currently not supported.`,
        );
        this.warnLog(
    ...
    if (existingAccessory) {
      ...
      if (await this.registerDevice(device)) {
        ...
        existingAccessory.displayName = device.configDeviceName
          ? await this.validateAndCleanDisplayName(
              device.configDeviceName,
              'configDeviceName',
              device.configDeviceName,
            )
          : await this.validateAndCleanDisplayName(
        ...
        existingAccessory.context.connectionType =
          await this.connectionType(device);
        existingAccessory.context.version =
          device.firmware ?? device.version ?? this.version ?? '0.0.0';
        await this.infoLog(
        ...
        await this.debugLog(
          `${device.deviceType} uuid: ${device.deviceId}-${device.deviceType}, (${existingAccessory.UUID})`,
        );
      } else {
      ...
      accessory.context.version =
        device.firmware ?? device.version ?? this.version ?? '0.0.0';
      const newOrExternal = !device.external
        ? 'Adding new'
        : 'Loading external';
      await this.infoLog(
        `${newOrExternal} accessory: ${accessory.displayName} deviceId: ${device.deviceId}`,
      );
      // create the accessory handler for the newly create accessory
      // this is imported from `platformAccessory.ts`
      new Humidifier(this, accessory, device);
      await this.debugLog(
        `${device.deviceType} uuid: ${device.deviceId}-${device.deviceType}, (${accessory.UUID})`,
      );
      ...
      await this.debugLog(
        `Device not registered: ${device.deviceName} ${device.deviceType} deviceId: ${device.deviceId}`,
      );
    }
  }
    ...
    if (existingAccessory) {
      ...
      if (await this.registerDevice(device)) {
        ...
        existingAccessory.context.model =
          SwitchBotModel.MeterPlusUS ?? SwitchBotModel.MeterPlusJP;
        existingAccessory.context.deviceId = device.deviceId;
        existingAccessory.context.deviceType = device.deviceType;
        existingAccessory.displayName = device.configDeviceName
          ? await this.validateAndCleanDisplayName(
    ...
    if (existingAccessory) {
      ...
      if (isBlindTiltDevice(device)) {
        if (device.group && !device.curtain?.disable_group) {
          ...
          if (device.master) {
            this.warnLog(
              `Main Curtain: ${device.deviceName}, deviceId: ${device.deviceId}`,
            );
          } else {
            this.errorLog(
              `Secondary Curtain: ${device.deviceName}, deviceId: ${device.deviceId}`,
            );
          }
        }
      }
  ...
  private async createCurtain(device: device & devicesConfig) {
    ...
    if (existingAccessory) {
      ...
      if (await this.registerDevice(device)) {
        ...
        existingAccessory.context.deviceType = device.deviceType;
        existingAccessory.context.model =
          device.deviceType === 'Curtain3'
            ? SwitchBotModel.Curtain3
  ...
  private async createPlug(device: device & devicesConfig) {
    ...
    if (existingAccessory) {
      ...
      if (await this.registerDevice(device)) {
        ...
        existingAccessory.context.deviceType = device.deviceType;
        existingAccessory.context.model =
          device.deviceType === 'Plug Mini (US)'
  ...
  private async createLock(device: device & devicesConfig) {
    ...
    if (existingAccessory) {
      ...
      if (await this.registerDevice(device)) {
        ...
        existingAccessory.context.deviceType = device.deviceType;
        existingAccessory.context.model =
          device.deviceType === 'Smart Lock Pro'
            ? SwitchBotModel.LockPro
            : SwitchBotModel.Lock;
        existingAccessory.displayName = device.configDeviceName
          ? await this.validateAndCleanDisplayName(
              device.configDeviceName,
  ...
  private async createCeilingLight(device: device & devicesConfig) {
    ...
    if (existingAccessory) {
      ...
      if (await this.registerDevice(device)) {
        ...
        existingAccessory.context.deviceType = device.deviceType;
        existingAccessory.context.model =
          device.deviceType === 'Ceiling Light Pro'
            ? SwitchBotModel.CeilingLightPro
            : SwitchBotModel.CeilingLight;
        existingAccessory.displayName = device.configDeviceName
          ? await this.validateAndCleanDisplayName(
              device.configDeviceName,
              'configDeviceName',
  ...
  private async createRobotVacuumCleaner(device: device & devicesConfig) {
    ...
    if (existingAccessory) {
      ...
      if (await this.registerDevice(device)) {
        ...
        existingAccessory.context.deviceType = device.deviceType;
        existingAccessory.context.model =
          device.deviceType === 'Robot Vacuum Cleaner S1'
  ...
  private async createTV(device: irdevice & devicesConfig) {
    const uuid = this.api.hap.uuid.generate(
      `${device.deviceId}-${device.remoteType}`,
    );
    ...
    if (!device.hide_device && existingAccessory) {
      ...
      await this.debugLog(
        `${device.remoteType} uuid: ${device.deviceId}-${device.remoteType}, (${existingAccessory.UUID})`,
      );
      ...
      await this.debugLog(
        `${device.remoteType} uuid: ${device.deviceId}-${device.remoteType}, (${accessory.UUID})`,
      );
      ...
      await this.debugLog(
        `Device not registered: ${device.deviceName} ${device.remoteType} deviceId: ${device.deviceId}`,
      );
    }
  }
  ...
  private async createAirConditioner(
    device: irdevice & devicesConfig & irDevicesConfig,
  ) {
    const uuid = this.api.hap.uuid.generate(
      `${device.deviceId}-${device.remoteType}`,
    );
  ...
  async registerCurtains(device: device & devicesConfig): Promise<boolean> {
    ...
    if (isCurtainDevice(device)) {
      await this.debugWarnLog(
        `deviceName: ${device.deviceName} deviceId: ${device.deviceId}, curtainDevicesIds: ${device.curtainDevicesIds},` +
      ...
      await this.debugWarnLog(
        `deviceName: ${device.deviceName} deviceId: ${device.deviceId}, blindTiltDevicesIds: ${device.blindTiltDevicesIds},` +
          ` master: ${device.master}, group: ${device.group}, disable_group: ${device.curtain?.disable_group},` +
          ` connectionType: ${device.connectionType}`,
      );
      registerWindowCovering = await this.registerWindowCovering(device);
  ...
  async registerWindowCovering(
    device:
      | ((curtain | curtain3) & devicesConfig)
      | (blindTilt & devicesConfig),
  ) {
    ...
    if (device.master && device.group) {
      ...
      await this.debugLog(
        `deviceName: ${device.deviceName} [${device.deviceType} Config] device.master: ${device.master},` +
          ` device.group: ${device.group} connectionType; ${device.connectionType}`,
      );
      await this.debugWarnLog(
        `Device: ${device.deviceName} registerCurtains: ${registerCurtain}`,
      );
      ...
      registerCurtain = true;
      await this.debugLog(
        `deviceName: ${device.deviceName} [${device.deviceType} Config] connectionType: ${device.connectionType}, ` +
          ` group: ${device.group}`,
      );
      await this.debugWarnLog(
        `Device: ${device.deviceName} registerCurtains: ${registerCurtain}`,
      ...
      await this.debugWarnLog(
        `Device: ${device.deviceName} registerCurtains: ${registerCurtain}, device.connectionType: ${device.connectionType}`,
      );
    }
    ...
    if (!device.hide_device && device.connectionType === 'BLE/OpenAPI') {
      switch (device.deviceType) {
        ...
        case 'Blind Tilt':
          ...
          await this.debugWarnLog(
            `Device: ${device.deviceName} ${device.deviceType} registerDevice: ${registerDevice}`,
          );
          break;
        default:
          registerDevice = true;
          await this.debugWarnLog(
            `Device: ${device.deviceName} registerDevice: ${registerDevice}`,
          );
      }
      if (registerDevice === true) {
        await this.debugWarnLog(
          `Device: ${device.deviceName} connectionType: ${device.connectionType}, will display in HomeKit`,
        );
      } else {
        await this.debugErrorLog(
          `Device: ${device.deviceName} connectionType: ${device.connectionType}, will not display in HomeKit`,
        );
      }
    } else if (
      !device.hide_device &&
      ...
      await this.debugWarnLog(
        `Device: ${device.deviceName} connectionType: ${device.connectionType}, will continue to display in HomeKit`,
      );
      ...
      registerDevice = false;
      await this.debugErrorLog(
        `Device: ${device.deviceName} hide_device: ${device.hide_device}, will not display in HomeKit`,
      );
    } else {
      registerDevice = false;
      await this.debugErrorLog(
  ...
  public async externalOrPlatformIR(
    device: device & irDevicesConfig,
    ...
    if (device.external) {
      await this.debugWarnLog(
        `${accessory.displayName} External Accessory Mode`,
      );
      this.externalAccessory(accessory);
    } else {
      await this.debugLog(
        `${accessory.displayName} External Accessory Mode: ${device.external}`,
      );
  ...
  public async externalOrPlatform(
  ...
  async statusCode(statusCode: number): Promise<void> {
    switch (statusCode) {
      ...
      case 160:
        await this.errorLog(
          `Command is not supported, statusCode: ${statusCode}, Submit Bugs Here: https://tinyurl.com/SwitchBotBug`,
        );
        break;
      case 161:
      ...
      case 190:
        await this.errorLog(
          `Requests reached the daily limit, statusCode: ${statusCode}`,
        );
        break;
      case 100:
        await this.debugLog(
          `Command successfully sent, statusCode: ${statusCode}`,
        );
        break;
      case 200:
      ...
      case 400:
        await this.errorLog(
          'Bad Request, The client has issued an invalid request. This is commonly used to specify validation errors in a request ' +
            `payload, statusCode: ${statusCode}`,
        );
        break;
      case 401:
        await this.errorLog(
          'Unauthorized,	Authorization for the API is required, but the request has not been authenticated, ' +
            `statusCode: ${statusCode}`,
        );
        break;
      case 403:
        await this.errorLog(
          'Forbidden,	The request has been authenticated but does not have appropriate permissions, or a requested resource is not ' +
            `found, statusCode: ${statusCode}`,
        );
        break;
      case 404:
        await this.errorLog(
          `Not Found,	Specifies the requested path does not exist, statusCode: ${statusCode}`,
        );
        break;
      case 406:
        await this.errorLog(
          'Not Acceptable,	The client has requested a MIME type via the Accept header for a value not supported by the server, ' +
            `statusCode: ${statusCode}`,
        );
        break;
      case 415:
        await this.errorLog(
          'Unsupported Media Type,	The client has defined a contentType header that is not supported by the server, ' +
            `statusCode: ${statusCode}`,
        );
        break;
      case 422:
        await this.errorLog(
          'Unprocessable Entity,	The client has made a valid request, but the server cannot process it. This is often used for ' +
            `APIs for which certain limits have been exceeded, statusCode: ${statusCode}`,
        );
        break;
      case 429:
        await this.errorLog(
          'Too Many Requests,	The client has exceeded the number of requests allowed for a given time window, ' +
            `statusCode: ${statusCode}`,
        );
        break;
      case 500:
        await this.errorLog(
          'Internal Server Error,	An unexpected error on the SmartThings servers has occurred. These errors should be rare, ' +
            `statusCode: ${statusCode}`,
        );
        break;
      default:
        await this.errorLog(
          `Unknown statusCode, statusCode: ${statusCode}, Submit Bugs Here: ' + 'https://tinyurl.com/SwitchBotBug`,
        );
    }
  }

  async retryRequest(
    deviceMaxRetries: number,
    deviceDelayBetweenRetries: number,
  ...
  async connectBLE(
    ...
    try {
      ...
      await this.debugLog(
        `${device.deviceType}: ${accessory.displayName} 'node-switchbot' found: ${switchbot}`,
      );
    } catch (e: any) {
      switchbot = false;
      await this.errorLog(
        `${device.deviceType}: ${accessory.displayName} 'node-switchbot' found: ${switchbot}, Error: ${e}`,
      );
    }
    return switchbot;
  }
    ...
    if (this.config.options) {
      ...
      if (this.config.options.delayBetweenRetries) {
        this.delayBetweenRetries =
          this.config.options.delayBetweenRetries * 1000;
        platformConfig.delayBetweenRetries =
          this.config.options.delayBetweenRetries;
      } else {
   ...
   */
  async validateAndCleanDisplayName(
    displayName: string,
    name: string,
    value: string,
  ): Promise<string> {
    ...
    if (!validPattern.test(value)) {
      this.warnLog(
        `WARNING: The accessory '${displayName}' has an invalid '${name}' characteristic ('${value}'). Please use only alphanumeric,` +
      ...
      if (invalidCharsPattern.test(value)) {
        this.warnLog(
          `Removing invalid characters from '${name}' characteristic`,
        );
      ...
      if (invalidStartEndPattern.test(value)) {
        this.warnLog(
          `Removing invalid starting or ending characters from '${name}' characteristic`,
        );
  ...
  async loggingIsDebug(): Promise<boolean> {
    return (
      this.platformLogging === 'debugMode' || this.platformLogging === 'debug'
    );
  }
