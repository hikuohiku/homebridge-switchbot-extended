...
export class AirConditioner extends irdeviceBase {
  ...
  constructor(
    ...
    this.HeaterCooler = {
      ...
      Service:
        accessory.getService(this.hap.Service.HeaterCooler) ??
        (accessory.addService(this.hap.Service.HeaterCooler) as Service),
      Active:
        accessory.context.Active ?? this.hap.Characteristic.Active.INACTIVE,
      CurrentHeaterCoolerState:
        accessory.context.CurrentHeaterCoolerState ??
        this.hap.Characteristic.CurrentHeaterCoolerState.IDLE,
      TargetHeaterCoolerState:
        accessory.context.TargetHeaterCoolerState ??
        this.hap.Characteristic.TargetHeaterCoolerState.AUTO,
    ...
    this.HeaterCooler.Service.setCharacteristic(
      this.hap.Characteristic.Name,
      this.HeaterCooler.Name,
    )
    ...
    this.HeaterCooler.Service.getCharacteristic(
    ...
    this.HeaterCooler.Service.getCharacteristic(
      this.hap.Characteristic.TargetHeaterCoolerState,
    )
      .setProps({
    ...
    this.HeaterCooler.Service.getCharacteristic(
      this.hap.Characteristic.CurrentHeaterCoolerState,
    ).onGet(async () => {
    ...
    if (this.device.irair?.meterType && this.device.irair?.meterId) {
      const meterUuid = this.platform.api.hap.uuid.generate(
        `${this.device.irair.meterId}-${this.device.irair.meterType}`,
      );
      this.meter = this.platform.accessories.find(
        (accessory) => accessory.UUID === meterUuid,
      );
      accessory.context.HumiditySensor = accessory.context.HumiditySensor ?? {};
      this.HumiditySensor = {
        Name: this.meter!.displayName,
        Service:
          this.meter!.getService(this.hap.Service.HumiditySensor) ??
          (this.meter!.addService(this.hap.Service.HumiditySensor) as Service),
        CurrentRelativeHumidity:
          this.meter!.context.CurrentRelativeHumidity || 0,
      };
    ...
    if (this.meter && this.HumiditySensor) {
      this.HumiditySensor.Service.setCharacteristic(
        this.hap.Characteristic.Name,
        this.HumiditySensor.Name,
      )
  ...
  async pushAirConditionerOnChanges(): Promise<void> {
    await this.debugLog(
      `pushAirConditionerOnChanges Active: ${this.HeaterCooler.Active},` +
        ` disablePushOn: ${this.disablePushOn}`,
    );
    if (
      this.HeaterCooler.Active === this.hap.Characteristic.Active.ACTIVE &&
      !this.disablePushOn
    ) {
  ...
  async pushAirConditionerOffChanges(): Promise<void> {
    await this.debugLog(
      `pushAirConditionerOffChanges Active: ${this.HeaterCooler.Active},` +
  ...
  async pushAirConditionerStatusChanges(): Promise<void> {
    await this.debugLog(
      `pushAirConditionerStatusChanges Active: ${this.HeaterCooler.Active},` +
        ` disablePushOff: ${this.disablePushOff},  disablePushOn: ${this.disablePushOn}`,
    );
    if (!this.Busy) {
      this.Busy = true;
      this.HeaterCooler.CurrentHeaterCoolerState =
        this.hap.Characteristic.CurrentHeaterCoolerState.IDLE;
    }
  ...
  async pushAirConditionerDetailsChanges(): Promise<void> {
    await this.debugLog(
      `pushAirConditionerDetailsChanges Active: ${this.HeaterCooler.Active},` +
        ` disablePushOff: ${this.disablePushOff},  disablePushOn: ${this.disablePushOn}`,
    );
  ...
  private async UpdateCurrentHeaterCoolerState() {
    if (this.HeaterCooler.Active === this.hap.Characteristic.Active.ACTIVE) {
      if (this.HeaterCooler.Active === undefined) {
        ...
        this.HeaterCooler.Active = this.HeaterCooler.Active
          ? this.HeaterCooler.Active
          : this.accessory.context.Active;
      }

      if (
        this.HeaterCooler.CurrentTemperature === undefined &&
        this.accessory.context.CurrentTemperature === undefined
      ) {
      ...
      if (
        this.HeaterCooler.ThresholdTemperature === undefined &&
        this.accessory.context.ThresholdTemperature === undefined
      ) {
      ...
      if (
        this.HeaterCooler.RotationSpeed === undefined &&
        this.accessory.context.RotationSpeed === undefined
      ) {
        this.HeaterCooler.RotationSpeed = 4;
      } else {
        this.HeaterCooler.RotationSpeed =
          this.HeaterCooler.RotationSpeed ||
          this.accessory.context.RotationSpeed;
      }
      ...
      if (this.meter) {
        if (
          this.HumiditySensor!.CurrentRelativeHumidity === undefined &&
          this.accessory.context.CurrentRelativeHumidity === undefined
        ) {
      ...
      if (
        this.HeaterCooler.ThresholdTemperature <
          this.HeaterCooler.CurrentTemperature &&
        this.HeaterCooler.TargetHeaterCoolerState !==
          this.hap.Characteristic.TargetHeaterCoolerState.HEAT
      ) {
        this.HeaterCooler.CurrentHeaterCoolerState =
          this.hap.Characteristic.CurrentHeaterCoolerState.COOLING;
      } else if (
        this.HeaterCooler.ThresholdTemperature >
          this.HeaterCooler.CurrentTemperature &&
        this.HeaterCooler.TargetHeaterCoolerState !==
          this.hap.Characteristic.TargetHeaterCoolerState.COOL
      ) {
        this.HeaterCooler.CurrentHeaterCoolerState =
          this.hap.Characteristic.CurrentHeaterCoolerState.HEATING;
      } else {
        this.HeaterCooler.CurrentHeaterCoolerState =
          this.hap.Characteristic.CurrentHeaterCoolerState.IDLE;
      }
      ...
      this.HeaterCooler.CurrentHeaterCoolerState =
        this.hap.Characteristic.CurrentHeaterCoolerState.INACTIVE;
    }
  }
  ...
  async pushChanges(bodyChange: any): Promise<void> {
    ...
    if (this.device.connectionType === 'OpenAPI' && !this.disablePushDetail) {
      await this.infoLog(
        `Sending request to SwitchBot API, body: ${bodyChange},`,
      );
      try {
  ...
  async CurrentTemperatureGet(): Promise<CharacteristicValue> {
    ...
    this.HeaterCooler.CurrentTemperature =
      this.accessory.context.CurrentTemperature || 24;
    await this.debugLog(
      `Get CurrentTemperature: ${this.HeaterCooler.CurrentTemperature}`,
    );
  ...
  async CurrentRelativeHumidityGet(): Promise<CharacteristicValue> {
    ...
    this.HumiditySensor!.CurrentRelativeHumidity =
      this.accessory.context.CurrentRelativeHumidity || 0;
    await this.debugLog(
      'Get' +
  ...
  async RotationSpeedGet(): Promise<number> {
    ...
    await this.debugLog(
      `Get RotationSpeed: ${this.HeaterCooler.RotationSpeed}`,
    );
  ...
  async TargetHeaterCoolerStateGet(): Promise<CharacteristicValue> {
    ...
    this.HeaterCooler.TargetHeaterCoolerState = this.ValidValues.includes(
      targetState,
    )
      ? targetState
      : this.ValidValues[0];
    await this.debugLog(
      `Get (${this.getTargetHeaterCoolerStateName()}) TargetHeaterCoolerState:` +
  ...
  async TargetHeaterCoolerStateSet(value: CharacteristicValue): Promise<void> {
    if (
      !this.hide_automode &&
      value === this.hap.Characteristic.TargetHeaterCoolerState.AUTO
    ) {
      ...
      this.errorLog(
        'Set TargetHeaterCoolerState: ' +
          `${this.HeaterCooler.TargetHeaterCoolerState}, hide_automode: ${this.hide_automode} `,
      );
    }
    this.pushAirConditionerStatusChanges();
  }
  ...
  async ThresholdTemperatureGet(): Promise<CharacteristicValue> {
    this.HeaterCooler.Active =
      this.HeaterCooler.Active === undefined
    ...
    await this.debugLog(
      `Get ThresholdTemperature: ${this.HeaterCooler.ThresholdTemperature}`,
    );
  ...
  async apiError(e: any): Promise<void> {
    this.HeaterCooler.Service.updateCharacteristic(
      this.hap.Characteristic.Active,
      e,
    );
    this.HeaterCooler.Service.updateCharacteristic(
